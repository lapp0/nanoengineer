...
def qt4info(msg, name = None, maxdepth = 1, browse = False):
    ...
    if type(msg) is type('abc'):
        ...
        print('Qt 4 INFO:', end=' ')
        if name is not None: print(name, end=' ')
        print(repr(msg))
        if browse:
...
def findDefiningClass(cls_or_method, method_name = None):
    ...
    if method_name is not None:
        if type(cls_or_method) is not type:
            cls_or_method = cls_or_method.__class__
        method = getattr(cls_or_method, method_name)
    elif type(cls_or_method) is types.MethodType:
        method = getattr(cls_or_method.__self__.__class__, cls_or_method.__func__.__name__)
    else:
        method = cls_or_method
    assert method.__self__ is None, "Method must be a class method, not an instance mthod"
    def hunt(klass, lst, depth, name = method.__func__.__name__, method = method):
        if hasattr(klass, name) and method.__func__ is getattr(klass, name).__func__:
            lst.append((depth, klass))
        for base in klass.__bases__:
            hunt(base, lst, depth + 1)
    lst = [ ]
    hunt(method.__self__.__class__, lst, 0)
    lst.sort()
    return lst[-1][1]
